				...
				<div class="row">
					...
					<div class="col-sm-6 content-group">
						<div class="invoice-details">
							...
							<ul class="list-condensed list-unstyled">
								<li>Date: <span class="text-semibold"><?php echo date('d-M-Y', strtotime($row->Sales_date)) ?></span></li>
							</ul>
							...
							<div class="table-responsive no-border">
								<table class="table">
									<tbody>
										...
										<tr>
											<th>Tax: <span class="text-regular"><?php if($row->Sales_tax_type=="Persentage"){ echo '('.$row->Sales_tax_amount.'%)'; }else{ echo '(Fixed)';} ?></span></th>
											<td class="text-right"><?php if($row->Sales_tax_type=="Fixed"){?><i class="fa fa-inr"></i>&nbsp;<?php echo $tax_amt = $row->Sales_tax_amount; }else{ echo '<i class="fa fa-inr"></i>&nbsp;'.$tax_amt = $subtotal*($row->Sales_tax_amount/100);}?></td>
										</tr>
										<tr>
											<th>Discount: <span class="text-regular"><?php if($row->Sales_dis_type=="Persentage"){ echo '('.$row->Sales_dis_amt.'%)'; }else{ echo '(Fixed)';} ?></span></th>
											<td class="text-right"><?php if($row->Sales_dis_type=="Fixed"){?><i class="fa fa-inr"></i>&nbsp;<?php echo  $row->Sales_dis_amt; }else{ $discount_amt = $tax_amt+$subtotal; echo '<i class="fa fa-inr"></i>&nbsp;'.$discount_amt*($row->Sales_dis_amt/100);}?></td>
										</tr>
										<tr>
											<th>Total:</th>
											<td class="text-right text-primary"><h5 class="text-semibold"><i class="fa fa-inr"></i>&nbsp;<?php echo $p_total = $row->Sales_total; ?></h5></td>
										</tr>
										<tr>
											<th>Paid (include advance amount <i class="fa fa-inr"></i>&nbsp;<?php  echo $row->Sales_advance; ?>):</th>
											<td class="text-right text-primary"><span class="text-regular"><i class="fa fa-inr"></i>&nbsp;<?php echo $p_paid = $row->Sales_paid; ?></span></td>
										</tr>

										<tr>
											<th>Balance Paid :</th>
											<td class="text-right text-primary"><h5 class="text-semibold"><i class="fa fa-inr"></i>&nbsp;<?php echo $p_balance = $row->Sales_balance; ?></h5></td>
										</tr>
									</tbody>
								</table>
							</div>
